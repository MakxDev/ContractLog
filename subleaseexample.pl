...
атрибуты_следующей_оплаты(НомерДоговора, ДатаОплаты, СуммаОплаты, Валюта) :-
	...
	format_time(atom(M), '%m', Сегодня), atom_string(M, MS), number_string(MN, MS), (MN =:= 12 -> МесяцОплаты = 1; МесяцОплаты is MN + 1),
	atomics_to_string([ГодОплаты, '-', МесяцОплаты, '-', ДеньОплаты], Str), atom_string(ДатаОплаты, Str).
	%atom_string(atom(ГодОплаты-МесяцОплаты-ДеньОплаты), ДатаОплаты).
	%D = date(ГодОплаты, МесяцОплаты, ДеньОплаты), format_time(ДатаОплаты, '%F', D). % !!! Ошибка
